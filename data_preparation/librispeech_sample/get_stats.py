...
if __name__ == '__main__':
    ...
    time_by_gender = get_histogram(records, lambda_key=lambda r: r.speaker.gender, lambda_value=lambda r: r.length / 16000)
    print('Time by gender, seconds', time_by_gender)

    time_by_subset = get_histogram(records, lambda_key=lambda r: r.speaker.subset, lambda_value=lambda r: r.length / 16000)
    print('Time by subset, seconds', time_by_subset)

    speaker_freq = get_histogram(records, lambda_key=lambda r: r.speaker.id, lambda_value=lambda r: 1)
    print('Number of uniq speakers', len(speaker_freq))

    book_lengths =  get_histogram(records, lambda_key=lambda r: r.book, lambda_value=lambda r: r.length)
    ...
    print(f'Book length disrtibution, seconds,  min: {min_length}, mean: {mean_length}, max: {max_length}; n_books={len(book_lengths)}')
